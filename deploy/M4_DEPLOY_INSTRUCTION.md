...
| 包 | 大小 | sha256 | 内容 |
|---|---|---|---|
| `qwb_m4_deploy_20260923.zip` | 见 SHA256SUMS | 见 `dist/SHA256SUMS_20260923.txt` | 代码 + Skill + UI + Golden 结果表 + 部署脚本 + mac 双击启动器（不含 `app/config.yaml`，不含任何 `.day`）。**含 2026-09-23 区间语义（方案A）修复** |
| `qwb_market_pack_20260923.zip` | 931.2 MB 原始 / 543.4 MB zip / 11705 文件 | `1eaaf4771a1661e546abc2a9049b76150a4147085c8d672b22a0249ede29c6f1` | 标准本地行情 `appdata/market`（11704 个 `.day`，截止 **2026-09-23**）+ `stock_names.csv` |
| `qwb_state_pack_20260923.zip` | 3.9 MB / 89 文件 | `24ef391698069f04052b54c77bc9babe32cc61794890bc0ec4249c17db2db0f1` | **工作台状态 `appdata/state`**：22 个模型定义 `state.json` + 88 个结果表文件（各模型 `current/previous` 的 `.json`/`.xlsx`）。含 2026-09-23 区间语义重跑后的 6 个模型结果（1S1A=148 行、10cm_v6=323 行、13b34ebf3a=214 行、f120ccad35 / 676e25bb91 / 99b11761e0 各 323 行），且库内 `plan.ranges` 已同步 |
...
> **2026-09-23 版（当前唯一有效版本）**：三个包均已对齐 **09-23 行情**、
> 10cm 新口径、以及**区间语义（方案A）修复**。旧 `*_20260921.zip` 两包、以及本轮之前的
> `qwb_market_pack_20260923.zip`（止于 09-22）**作废**。以 `sha256` 全量校验为准。
> 行情包本次改为 deflate 压缩交付（内容逐字节不变，体积 932.9 MB → 543.4 MB，便于传输）。

**行情数据截止 2026-09-23**。两端行情必须**完全一致**，否则 P9 第一道关（输入资产哈希）
...
后端已有的 22 个模型不会自动全部成卡 —— 可在界面里手动新建卡片并绑定模型）。
...
- 三包解压后，**历史行情完全离线自足**（11704 个 `.day`，含沪深主板/创业板/科创板/北交所，
  截止 2026-09-23）：回测、复核、golden 对账都不需要网络。
...
- **2026-09-23 区间语义裁定（方案A，重要，kk 定稿）**：规则原文里 `D-1` 与 `D-x` 写了
  **同一组条件**时，语义为「`D-x~D-1` **整段区间每一天**都必须满足该组条件」，而不是只校验两个端点。
  - 计划语言新增 `strategy_plan.ranges`（`[{"from":{"var":0},"to":-1,"conds":{…},"label":"D-x~D-1"}]`）；
    执行器按区间整段判定（前缀和 O(1)），复核器逐日独立重验（新增 `RANGE_*` 违例码）。
  - 识别器两种来源：**显式写法**（`D-x~D-1：全部不是涨停，成交额不是最大，股价不是最高`）
    与**自动升级**（`D-1` / `D-x` 同组条件 + 输出含以 `D-x` 为一端的区间指标）。识别回执会显式
    写明「已按区间语义翻译」，`plan_summary.notes` 同样留痕（可审计，非隐式推断）。
  - 触发根因示例：`603156 @ 2026-05-18` 的整理区间 `2026-05-08 ~ 05-15` 内 **2026-05-13 封死涨停**
    （前收 43.02 → 涨停价 `43.02×1.1=47.322→47.32`，当日最高=收盘=47.32），端点式放行了该票。
  - 影响：本机 6 个模型行数变化 —— 1S1A `274→148`、10cm_v6 `622→323`、
    f120ccad35/676e25bb91/99b11761e0 `55→323`（这三个库内计划为历史遗留错位翻译，本轮一并纠正）、
    13b34ebf3a `377→214`；全部复核 0 违例。**M4 侧若已有这些模型的旧结果，请重跑**
    （每日管线只在数据日期变化时才自动重跑）。三包已携带新结果，通常无需手动重跑。
  - 与 kk 参考的逐键比对（同一份 09-23 行情）：1S1A 的 148 行 = kk `0923_backtest` 的 **131 行
    （交集 131 / 漏 0 / 数值真差异 0）** + **17 行创业板**（kk 参考脚本池不含 300）；
    300 创业板由 31 → 17（剔除 14 个假阳性，新增 0，全部落在 2020-08-24 前的 10cm 时代）。
  - 活体验收：对运行中的 `127.0.0.1:8000` 发真实 HTTP 请求，识别回执含 `ranges`、回测得 **148 行**；
    用变更前识别器重建的端点式计划回测得 **274 行**（可复现修复前值）。
  - golden 26 例（全为固定日规则，计划不变）**逐字段零回归**（同数据双跑对照实证）；
    P9 skill/golden/stock_names 三个输入哈希不变，仅 `market_pack` 随 09-23 行情变化（两端同步即一致）。
  - 证据与明细：包内 `artifacts/verification/1s1a_bug_0923/`（`CHANGES.md` 变更记录、
    `ROOT_CAUSE.md` 根因报告、`live_api_check.*` 活体验收、`diff_vs_0923_ref.*` / `gem_breakdown.txt`
    差集核算、`p9_before_after_compare.txt` P9 新旧对照）。
- 三包对齐：代码包（10cm 新口径 + 区间语义 + mac 启动器）/ 行情包（`.day` 截止 **09-23**）/
  **state 包（22 个模型 + 结果表，含区间语义新结果）**。
